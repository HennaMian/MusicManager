...
# load the feature matrix (NxC)
feature_matrix = np.load("./simp6/data_top7_n3_clean.npy", allow_pickle=True)

    
# load the genre truth values (N)
genre_truths = np.load("./simp6/labels_top7_n3_clean.npy", allow_pickle=True)




# load the feature matrix (NxC)
# feature_matrix = np.load("./simp1/music_features_matrix_reduced_features_top10_n10.npy", allow_pickle=True)

    
# # load the genre truth values (N)
# genre_truths = np.load("./simp1/music_feature_names_reduced_features_top10_n10.npy", allow_pickle=True) 
...
# find_optimal_num_clusters(feature_matrix)

def generateTable(labelsSelected):
    ...
    for i in range(k):
        ...
        for point in index:
            ...
            elif ((dist < minDist) and (genre_truths[point] not in finalLabels)):
                minDist = dist
                minVal = point
        ...
        finalLabels.append(genre_truths[minVal])
    ...
    return occurance, countss, accuracy
...
if len(sys.argv) > 1:
    labelsSelected = sys.argv[1]

labelsSelected = int(labelsSelected)    

rows = 0
for x in range(2, labelsSelected+1):
    rows+=x
rows+=1
rows+=(labelsSelected-1)
    

mdFile = MdUtils(file_name='KMeansResultsNew6')

list_of_strings = ["Labels Selected", "Genre", "Occurances", "Accuracy"]

for i in range(2, labelsSelected+1):
    occurance, countss, accuracy = generateTable(i)
    # print(occurance)
    # print(countss)
    # print(accuracy)
    # if (i>=9):
    #     occurance, countss, accuracy = generateTable(i) 
    list_of_strings.extend([str(i), str(occurance[0]), str(countss[0]), str(accuracy[0])])


    for line in range(1, i):
        list_of_strings.extend(["", str(occurance[line]), str(countss[line]), str(accuracy[line])])
...
mdFile.new_table(columns=4, rows=rows, text=list_of_strings, text_align='center')
